...
export default function StoreAboutPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { AboutPage } = getTemplate(store.template);

  return (
    <StoreShell store={store} slug={slug}>
      <AboutPage slug={slug} store={store} products={products} />
    </StoreShell>
  );
}
